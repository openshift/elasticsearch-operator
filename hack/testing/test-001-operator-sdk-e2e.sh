...
source "$repo_dir/hack/lib/util/misc.sh"
...
if [ -n "${IMAGE_FORMAT:-}" ] ; then
  IMAGE_ELASTICSEARCH_OPERATOR=$(gnu_sed -e "s,\${component},elasticsearch-operator," <(echo $IMAGE_FORMAT))
fi
...
gnu_sed -i "s,quay.io/openshift/origin-elasticsearch-operator:latest,${IMAGE_ELASTICSEARCH_OPERATOR}," ${manifest}
...
gnu_sed -i "s/namespace: openshift-logging/namespace: ${TEST_NAMESPACE}/g" ${manifest}
